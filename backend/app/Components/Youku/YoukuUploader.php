...
class YoukuUploader
{
    ...
    private function versionUpdate($verlog)
    {
        $file = @fopen($verlog, "r");
        if (!$file) {
            LogHelper::info("Could not open " . $verlog . "!");
        } else {
            $version = trim(fgets($file));
            LogHelper::info("Your current sdk version is:" . $version);
            $param = array(
                'client_id' => $this->client_id,
                'version' => $version,
                'type' => 'php'
    ...
    private function writeRefreshFile($refresh_file, $refresh_json_result)
    {
        ...
        if (!$file) {
            LogHelper::error("Could not open " . $refresh_file . "!");
        } else {
            $refreshInfo = json_encode($refresh_json_result);
            $fw = @fwrite($file, $refreshInfo);
            if (!$fw) {
                LogHelper::error("Write refresh file fail!");
            }
            fclose($file);
        }
    }
    ...
    public function upload($upload_process = true, $params = array(), $uploadInfo = array())
    {
        if (isset($params['access_token']) && !empty($params['access_token'])) {
            ...
            LogHelper::info("Only applys to the clients of partner level!\n");
        ...
        LogHelper::info("Uploading start!\n");
        $finish = false;
        ...
        do {
            ...
            if ($upload_process) {
                LogHelper::info("Upload progress:{$transferred}%\n");
            }
        } while (!$finish);
        if ($finish) {
            $commitResult = $this->commit($uploadServerIp);
            LogHelper::info("Uploading success!\n");
            if (isset($commitResult->video_id)) {
                return $commitResult->video_id;
            }
        }
        return null;
    }
